...
export const getPosts = async(req,res) => {
    ...
    try {
        ...
        res.status(200).json({ data: creatorPosts, currentPage: Number(page), numberOfPages: Math.ceil(total / LIMIT) });
    } catch (error) {
...
export const getPostsByAdmin = async(req,res) => {
    ...
    try {
        const startIndex = (Number(page) - 1) * LIMIT; //gettting the start index of every page
        const total = await PostMessage.countDocuments({});
        const posts= await PostMessage.find({isAdminPost: true}).sort({ createdAt: -1 }).limit(LIMIT).skip(startIndex);
...
export const getPostsBySearch = async (req, res) => {
    ...
    try {
        ...
        const posts = await PostMessage.find({ $or: [ { title } ,  { tags: { $in: tags.split(',') } } ] }).sort({ isAdminPost: -1, createdAt: -1 });
        res.status(200).json({ data: posts });
    } catch (error) {
        res.status(404).json({ message: error.message});
    }
...
export const createPost = async(req,res) => {
    ...
    const newPost = new PostMessage({ ...post, creator: req.userId, isAdminPost: req.isAdmin, createdAt: new Date().toISOString(), createdDate});
    try {
        const existingTag = await PostMessage.find({ $and: [{ creator: req.userId}, {tags: post.tags[0]}, {createdDate}] })
        if (existingTag[0]) {
            return res.status(404).json({message: 'This challenge is already done by you today. Try again tomorrow.'});
        }
        await newPost.save();
